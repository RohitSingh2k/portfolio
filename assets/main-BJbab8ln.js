(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&a(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();function w(){document.getElementById("year").textContent=new Date().getFullYear()}function g(e,s,t=50){return new Promise(a=>{let n=0;e.textContent="";function o(){n<s.length?(e.textContent+=s.charAt(n),n++,setTimeout(o,t)):a()}o()})}function E(e){const s=document.getElementById("hero-name"),t=document.getElementById("hero-role"),a=document.getElementById("hero-tagline");s.textContent="",t.textContent="",a.textContent="",g(s,e.profile.name,100).then(()=>g(t,e.profile.role,50)).then(()=>g(a,e.profile.tagline,30)),document.getElementById("hero-image").src=e.profile.dpPath,document.getElementById("about-bio").innerHTML=`<p>${e.profile.bio}</p>`,document.getElementById("contact-email")&&(document.getElementById("contact-email").href=`mailto:${e.profile.email}`);const n=document.getElementById("hero-socials"),o=document.getElementById("footer-socials");n.innerHTML="",o.innerHTML="",e.socials.forEach(r=>{const i=document.createElement("a");i.href=r.url,i.target="_blank",i.rel="noopener noreferrer",i.className="text-slate-400 hover:text-primary transition-colors text-2xl",i.innerHTML=`<i class="${r.icon}"></i>`,n.appendChild(i),o.appendChild(i.cloneNode(!0))});const l={},y=document.getElementById("skills-container");y.innerHTML="",e.skills.forEach(r=>{l[r.name.toLowerCase()]=r.icon;const i=document.createElement("div");i.className="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-700 rounded-full text-sm font-medium hover:bg-white hover:text-primary transition-all border border-slate-200 shadow-sm hover:shadow-md cursor-default";const c=r.icon||"fas fa-code";i.innerHTML=`
            <i class="${c} text-lg"></i>
            <span>${r.name}</span>
        ...
        `;c.innerHTML=m,d.innerHTML+=m});const f=document.getElementById("projects-container");f&&e.projects&&(f.innerHTML="",e.projects.forEach((r,i)=>{const c=i%3===0?"delay-100":i%3===1?"delay-200":"delay-300",p=r.responsibilities.map(h=>`<li class="flex items-start mb-2">
            ...
            `;f.innerHTML+=m}));try{window.jQuery&&window.jQuery().ripples&&$("#home").ripples({resolution:512,dropRadius:20,perturbance:.04})}catch{console.warn("Ripples library not loaded or failed to initialize")}}let u=null,C=null;const v="/portfolio/";async function L(e){try{if(!u){const o=await fetch(`${v}config.json`);if(!o.ok)throw new Error("Failed to load config.json");u=await o.json()}const s=await fetch(`${v}assets/i18n/${e}.json`);if(!s.ok)throw new Error(`Failed to load language: ${e}`);const t=await s.json(),a={...u,...t,profile:{...u.profile,...t.profile}};C=a,document.documentElement.lang=e,document.querySelectorAll("[data-i18n]").forEach(o=>{const l=o.getAttribute("data-i18n");a.ui&&a.ui[l]&&(o.textContent=a.ui[l])});const n=document.getElementById("current-lang-label");n&&(n.textContent=e==="en"?"EN":"HI"),E(a)}catch(s){console.error("Error loading language:",s)}}window.switchLanguage=function(e){L(e);const s=document.getElementById("mobile-menu");s&&s.classList.add("hidden")};function T(){const e=document.getElementById("mobile-menu-btn"),s=document.getElementById("mobile-menu");!e||!s||(e.addEventListener("click",()=>{s.classList.toggle("hidden");const t=e.querySelector("i");s.classList.contains("hidden")?(t.classList.remove("fa-times"),t.classList.add("fa-bars")):(t.classList.remove("fa-bars"),t.classList.add("fa-times"))}),s.querySelectorAll("a").forEach(t=>{t.addEventListener("click",()=>{s.classList.add("hidden"),e.querySelector("i").classList.remove("fa-times"),e.querySelector("i").classList.add("fa-bars")})}))}function B(){const e=document.getElementById("contact-form");e&&e.addEventListener("submit",function(s){s.preventDefault();const t=e.querySelector('button[type="submit"]'),a=t.innerHTML;t.innerHTML='<i class="fas fa-spinner fa-spin mr-2"></i> Sending...',t.disabled=!0;const n=new FormData(e);fetch(e.action,{method:"POST",body:n,headers:{Accept:"application/json"}}).then(o=>{if(o.ok)e.reset(),t.innerHTML='<i class="fas fa-check mr-2"></i> Sent Successfully!',t.classList.remove("bg-primary","hover:bg-secondary"),t.classList.add("bg-green-600","hover:bg-green-700"),setTimeout(()=>{t.innerHTML=a,t.disabled=!1,t.classList.add("bg-primary","hover:bg-secondary"),t.classList.remove("bg-green-600","hover:bg-green-700")},3e3);else throw new Error("Form submission failed")}).catch(o=>{console.error("Error:",o),t.innerHTML='<i class="fas fa-exclamation-circle mr-2"></i> Failed to Send',t.classList.remove("bg-primary","hover:bg-secondary"),t.classList.add("bg-red-600","hover:bg-red-700"),setTimeout(()=>{t.innerHTML=a,t.disabled=!1,t.classList.add("bg-primary","hover:bg-secondary"),t.classList.remove("bg-red-600","hover:bg-red-700")},3e3)})})}document.addEventListener("DOMContentLoaded",()=>{L("en"),T(),w(),B()});
